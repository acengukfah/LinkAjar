...
public final class KelasDao_Impl implements KelasDao {
  ...
  public KelasDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfSiswaModel = new EntityInsertionAdapter<SiswaModel>(__db) {
      @Override
      public String createQuery() {
        return "INSERT OR ABORT INTO `tb_siswa`(`id_siswa`,`id_kelas`,`nama_siswa`,`umur`,`jenis_kelamin`,`asal`,`nomor_hp`,`email`) VALUES (nullif(?, 0),?,?,?,?,?,?,?)";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, SiswaModel value) {
        ...
        if (value.getNomor_hp() == null) {
          stmt.bindNull(7);
        } else {
          stmt.bindString(7, value.getNomor_hp());
        }
    ...
    this.__updateAdapterOfSiswaModel = new EntityDeletionOrUpdateAdapter<SiswaModel>(__db) {
      @Override
      public String createQuery() {
        return "UPDATE OR ABORT `tb_siswa` SET `id_siswa` = ?,`id_kelas` = ?,`nama_siswa` = ?,`umur` = ?,`jenis_kelamin` = ?,`asal` = ?,`nomor_hp` = ?,`email` = ? WHERE `id_siswa` = ?";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, SiswaModel value) {
        ...
        stmt.bindLong(9, value.getId_siswa());
      }
    };
  }
  ...
  @Override
  public List<SiswaModel> detailSiswa(int id_siswa) {
    final String _sql = "SELECT * FROM TB_SISWA WHERE id_siswa = ? LIMIT 1";
    final RoomSQLiteQuery _statement = RoomSQLiteQuery.acquire(_sql, 1);
    int _argIndex = 1;
    _statement.bindLong(_argIndex, id_siswa);
    final Cursor _cursor = __db.query(_statement);
    try {
      ...
      final int _cursorIndexOfNomorHp = _cursor.getColumnIndexOrThrow("nomor_hp");
      ...
      while(_cursor.moveToNext()) {
        ...
        final String _tmpNomor_hp;
        _tmpNomor_hp = _cursor.getString(_cursorIndexOfNomorHp);
        _item.setNomor_hp(_tmpNomor_hp);
